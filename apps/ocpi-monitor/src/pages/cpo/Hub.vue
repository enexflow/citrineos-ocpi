  ...
  <div>
    <div class="mb-6 d-flex flex-wrap align-center justify-space-between ga-3">
      <div>
        ...
        <p class="text-body-2 mb-0" style="color: #587474">
          Roaming partners relayed through a hub. Open one to see its own
          tokens, transactions and CDRs.
        </p>
      </div>
    ...
    <v-card class="export-card mb-6">
      ...
      <v-card-subtitle class="text-wrap" style="color: #587474">
        Every CDR sent through this hub, across all roaming partners, whose
        session overlaps the selected period — a CDR starting before or
    ...
    >
      ...
      <template #no-data>
        <div class="py-8 text-center text-medium-emphasis">
          No roaming partners registered for this hub yet.
        </div>
      </template>
    </v-data-table>
  </div>
...
<script lang="ts" setup>
  import type { HubCpoView, HubOverview, RoamingEmspSummary } from '@/types/partner'
  import { computed, onMounted, ref, watch } from 'vue'
  import { useRouter } from 'vue-router'
  import { fetchHubCpoCdrExport, fetchHubCpoView, fetchHubs } from '@/api/hub'
  ...
  const detail = ref<HubCpoView | null>(null)
  ...
  const headers = [
    ...
    { title: 'CDRs sent', key: 'cdrCount' },
    { title: 'Energy (kWh)', key: 'totalEnergyKwh' },
    { title: 'Total cost', key: 'costLabel', sortable: false },
    { title: '', key: 'actions', sortable: false },
  ]

  const rows = ref<Array<RoamingEmspSummary & { costLabel: string }>>([])

  watch(detail, value => {
    rows.value = (value?.roamingEmsps ?? []).map(r => ({
      ...r,
      costLabel:
  ...
  function openRoamingPartner (item: RoamingEmspSummary) {
    router.push({
  ...
  function onRowClick (_event: unknown, row: { item: RoamingEmspSummary }) {
  ...
  async function exportExcel () {
    ...
    try {
      const groups = await fetchHubCpoCdrExport(
        selectedHubId.value,
  ...
  async function loadDetail (hubId: number) {
    ...
    error.value = null
    try {
      detail.value = await fetchHubCpoView(hubId)
    } catch (error_) {
      detail.value = null
      error.value = error_ instanceof Error ? error_.message : String(error_)
    } finally {
