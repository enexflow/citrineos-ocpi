...
export default {
  up: async (queryInterface: QueryInterface) => {
    // add disableOCPI to the notify payload so consumers don't have to query it
    await queryInterface.sequelize.query(`
      CREATE OR REPLACE FUNCTION "LocationNotify"()
      RETURNS trigger AS $$
      DECLARE
        requiredFields text[] := ARRAY['id', 'tenantId', 'updatedAt', 'ownerTenantPartnerId', 'disableOCPI'];
        requiredData jsonb;
        changedData jsonb;
  ...
  down: async (queryInterface: QueryInterface) => {
    await queryInterface.sequelize.query(`
      ...
      RETURNS trigger AS $$
      DECLARE
        requiredFields text[] := ARRAY['id', 'tenantId', 'updatedAt', 'ownerTenantPartnerId'];
        requiredData jsonb;
        changedData jsonb;
        notificationData jsonb;
        tenantData jsonb;
