...
describe('TokensMapper', () => {
  describe('toDto (Authorization -> TokenDTO)', () => {
    ...
    it('should fall back to tenant when tenantPartner is null (own tokens)', () => {
      const ownAuth = {
        ...mockAuthorization,
        tenantPartner: null,
        tenant: {
      ...
      expect(result.country_code).toBe('FR');
      expect(result.party_id).toBe('ZTA');
      expect(result.uid).toBe('TOKEN001');
    });

    it('should prefer tenantPartner over tenant when both are present', () => {
      const authWithBoth = {
        ...mockAuthorization,
        tenant: {
          countryCode: 'DE',
      ...
      const result = TokensMapper.toDto(
        authWithBoth as unknown as AuthorizationDto,
      );
